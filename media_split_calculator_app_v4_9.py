...
def allocate_budget(df, total_budget=240, alpha=1.6, beta=1.0, other_share=10.0):
    ...
    total_margin = (df_valid['contribution'].sum() / df_valid['recommended budget'].sum()) * 100 if not df_valid.empty else 0

    return df_final, summary, total_margin


st.set_page_config(page_title='Media Split Calculator v4.9', layout='wide')
st.title('📊 Media Split Calculator — Fixed Bounds (v4.9)')
...
if st.session_state.mode == 'default':
    ...
    df_result, summary, total_margin = allocate_budget(df_to_use, total_budget, alpha, beta, other_share)
    ...
    # Вывод маржинальности
    if total_margin is not None:
        st.markdown(f"### 💰 Общая маржинальность сплита: **{total_margin:.2f}%**")

    csv = df_result.to_csv(index=False).encode('utf-8')
    st.download_button('💾 Download Result (CSV)', data=csv, file_name='media_split_result_v4_9.csv', mime='text/csv')
